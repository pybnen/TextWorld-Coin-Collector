...
class LSTM_DQN(torch.nn.Module):
    ...
    def __init__(self, model_config, word_vocab, commands, enable_cuda=False):
    ...
    def _def_layers(self):
        ...
        self.action_scorer_command = torch.nn.Linear(self.action_scorer_hidden_dim, self.n_commands, bias=False)
        self.fake_recurrent_mask = None

    def init_weights(self):
        torch.nn.init.xavier_uniform_(self.action_scorer_shared.weight.data, gain=1)
        torch.nn.init.xavier_uniform_(self.action_scorer_command.weight.data, gain=1)
    ...
    def recurrent_action_scorer(self, state_representation, last_hidden=None, last_cell=None):
        ...
        command_rank = self.action_scorer_command.forward(new_h)  # batch x n_commands
        return command_rank, new_h, new_c
    ...
    def action_scorer(self, state_representation):
        ...
        command_rank = self.action_scorer_command.forward(hidden)  # batch x n_commands
        return command_rank
